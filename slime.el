...
function name is prompted."
  (interactive (list (slime-read-symbol-name (format "Edit definition of (in %s): "
                                                     (slime-current-package)))))
...
RESTARTS should be a list ((NAME DESCRIPTION) ...)."
  (let* ((len (length restarts))
    ...
    (loop with longest-restart-name-length =
               (loop
                  for (name nil) in (subseq restarts start end)
                  maximize (length name))
          for (name string) in (subseq restarts start end)
  ...
  (let ((case-fold-search t)
    ...
    (or (loop
           ...
           do (setf winner idx)
                     ...
                     (loop
                        for frame in (subseq frames (1+ winner))
      ...
      (slime-propertize-region `(frame ,frame details-visible-p t)
        ...
        (let ((indent1 "      ")
          ...
          (when (and (not catches)
                     (not locals))
            (insert indent1 (in-sldb-face detailed-frame-line "[Nothing here]") "\n"))
          (setq end (point)))))
...
VAR should be a plist with the keys :name, :id, and :value."
  (let ((max-name-length
         (loop for var in vars
               maximize (destructuring-bind (&key name id value) var
                          (length (concat name (if (zerop id)
                                                   ""
                                                   (format "#%d" id))))))))
    (loop for i from 0
          for var in vars do
          (destructuring-bind (&key name id value) var
            (slime-propertize-region (list 'sldb-default-action 'sldb-inspect-var
                                           'var i)
              (let ((name (concat name (if (zerop id) "" (format "#%d" id)))))
                (insert prefix
                        (in-sldb-face local-name name)
...
See `def-slime-selector-method' for defining new methods."
  ...
  (let* ((slime-selector-other-window other-window)
         (method (find key slime-selector-methods :key #'car)))
    (cond (method 
...
(pushnew (list ?4 "Select in other window" (lambda () (slime-selector nil t)))
